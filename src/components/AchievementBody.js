...
function Body(props) {
  ...
  const timer = useRef("");
  const audio = new Audio("http://localhost:8080/subtitle/audio/Result");
  let videoId;
  if (currentProject) {
    videoId = currentProject.learnResourceLink.match(/\w+:\/\/[^/:]+[^# ]*v=([\w\-]+)/)[1];
  } else {
    videoId = JSON.parse(window.localStorage.getItem("currentProject")).learnResourceLink.match(/\w+:\/\/[^/:]+[^# ]*v=([\w\-]+)/)[1];
  }
  //拿字幕資料
  useEffect(() => {
    CallApi('subtitle/eachClassLearnProject/', setStates, '', JSON.parse(window.localStorage.getItem("currentProject")).id);
    ...
    if (player && states) {
      const videoStates = (e) => {
        if (e.data === 1) {
          ...
          console.log('影片暫停撥放, videoState.current =' +videoState.current )
          audio.pause();
          clearTimeout(timer.current)
        }
      }
      if(videoState.current === 0) {
        player.addEventListener('onStateChange', videoStates)
        videoState.current = 1
      }
      return ()=>{
        player.removeEventListener('onStateChange', videoStates)
      }
  }
  }, [player,states])
  // 字幕時間轉換秒數
  const time = (t) => {
    return Number(t.slice(3, 5)) * 60 + Number(t.slice(6, 8)) + Number(t.slice(9, 11)) * 0.001;
  };
  //判斷顯示字幕
  let state
    const subtitleSearch = (e,sss) => {
      ...
      }else if (sss.filter(s => time(s.startTime) < crttentTime && time(s.endTime) > crttentTime).length === 0) {
        state = sss.filter(s => time(s.startTime) > crttentTime)[0]
      }else {
        state = sss.filter(s => time(s.startTime) < crttentTime && time(s.endTime) > crttentTime)[0]
      }
      if (state) {
        setData(state)
        console.log("play")
        ...
        } else if(time(state.startTime) < player.getCurrentTime() && !showSubtitle){
          console.log("222")
          setShowSubtitle(true);
          timer.current = setTimeout(() => {
    ...
    //重覆更新字幕
  useEffect(()=>{if(states){subtitleSearch(0,states)}},[showSubtitle])
  ...
  return (
    <Container>
      ...
      <Control>
        ...
        />
        <p>台文</p>
        <SwitchButton clicked={() => setSwitchOne(!switchOne)} active={switchOne} />
        <p>華文</p>
        <SwitchButton clicked={() => setSwitchTwo(!switchTwo)} active={switchTwo} />
        <p>字幕位置</p>
        <SwitchButton clicked={() => setSwitchThree(!switchThree)} active={switchThree} width={43} text={1} color='#90a3b1' />
      </Control>
      <ControlBk />
      <InfoContainer>
        <Info>
