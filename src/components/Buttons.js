...
function Buttons(props) {
  ...
  useEffect(() => {
    ...
    //   ) {
    //     setaudioSRC(
    //       "http://https://7acd-61-222-207-205.ngrok-free.app:8081" + states[order].subtitleVoiceLink + ""
    //     );
    //   } else {
    //     setaudioSRC(
    //       "http://localhost:8081" +
  ...
  function checkLink(subtitleVoiceLink) {
    if (subtitleVoiceLink.includes("type")) {
      return "https://7acd-61-222-207-205.ngrok-free.app" + subtitleVoiceLink + ""
    } else {
      return "https://7acd-61-222-207-205.ngrok-free.app" + subtitleVoiceLink + "/type/test"
    }
  }
  // 定義按鈕對應的函式
  const func = [
